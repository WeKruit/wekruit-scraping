...
- `/Users/spencerwang/Documents/GitHub/wekruit-pa` is cloned permanently. The package-publish PR `https://github.com/WeKruit/wekruit-pa/pull/88` has been merged to PA `main`; local PA `main` is clean and synced with `origin/main`.
...
- Current package findings:
  ...
  - Registry verification passed: `npm view @wekruit/shared-tags name version dist.tarball --registry=https://npm.pkg.github.com` returned name `@wekruit/shared-tags`, version `0.1.0`, and the tarball URL above.
  - Clean temp consumer verification passed using normal npm registry resolution plus the user-level `@wekruit` GitHub Packages scope mapping: `npm install @wekruit/shared-tags@0.1.0`, then ESM imports from `@wekruit/shared-tags/canonical` and the main `@wekruit/shared-tags` barrel printed `install/import smoke ok 17 64`.
  - PA git status stayed clean after publish and verification: `main...origin/main`.
  ...
  - Before T0, core-service used npm lockfile v3 and its sourcing deploy bundle wrote `deploy/sourcing-functions/package.json`, copied `package-lock.json`, ignored `node_modules`, and had no `.npmrc`/scope mapping in either root or deploy bundle. Private package registry mapping and token injection therefore needed to be verified in the same shape Firebase deploy uses before depending on the package in deployed functions.
  ...
  - 2026-05-16 PA package patch branch/PR:
    - Created PA branch `codex/shared-tags-cjs-0.1.1` from clean synced PA `main`; do not merge this branch directly.
...
1. Phase T0 - Package publishing/auth preflight.
   ...
   - 2026-05-16 planning re-review after publish:
     - Before T0, local user-level npm config mapped `@wekruit` to `https://npm.pkg.github.com` and had protected auth for `Spec700`, but the core-service repo had no committed `.npmrc` and the generated `deploy/` directory was gitignored. T0 resolved the committed root scope mapping and generated deploy-bundle scope mapping.
